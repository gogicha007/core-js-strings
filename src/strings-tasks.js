...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) >= 0) {
    newStr = [...str];
    newStr.splice(str.indexOf(value), value.length);
    newStr = newStr.join('');
    return newStr;
  }
  return str;
...
function removeLastOccurrences(str, value) {
  ...
  if (str.lastIndexOf(value) >= 0) {
    newStr = [...str];
    newStr.splice(str.lastIndexOf(value), value.length);
    newStr = newStr.join('');
    return newStr;
  }
  return str;
...
function countVowels(str) {
  ...
  return [...str].reduce((acc, val) => acc + vowels.includes(val), 0);
}
...
function isPalindrome(str) {
  const lowCase = str.toLowerCase().replace(/[^A-Za-z0–9_]/g, '');
  const reverse = lowCase.split('').reverse().join('');
